...
<style>
    ...
    * {
    ...
    .ui-autocomplete-product .product-code {
        color: var(--dark-gray);
        font-weight: bold;
        margin-right: 10px;
    }
    .ui-autocomplete-product .product-name {
        color: var(--secondary-color);
    }
    .ui-autocomplete-product .product-price {
        float: right;
        color: var(--success-color);
    }
    /* Apenas este novo estilo para aumentar o nome do cliente selecionado */
    #cliente_info {
        font-size: 1.6em;
        color: #3a56d4;
        font-weight: 500;  /* Deixei um pouco mais negrito */
        margin-top: 8px;  /* Aumentei o espaçamento superior */
    }
</style>
</head>
<body>
                ...
                <div class="table-responsive">
                    <table id="itens">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td>
                                    <input type="text" class="form-control produto-search" 
                                           placeholder="Digite código ou nome do produto..."
                                           value="<%= item.get("produto_id") + " - " + item.get("nome_produto")%>">
                                    <select name="produto_id[]" class="hidden-select" onchange="atualizarPreco(this)" required>
    ...
    <script>
                        ...
                        function validarFormulario() {
                            ...
                            for (let select of produtoSelects) {
                                if (select.value === "") {
                                    alert('Por favor, selecione um produto para todos os itens antes de salvar o pedido.');
                                    select.closest('tr').querySelector('.produto-search').focus();
                                    select.closest('tr').querySelector('.produto-search').style.borderColor = 'red';
                                    setTimeout(() => {
                                        select.closest('tr').querySelector('.produto-search').style.borderColor = '';
                                    }, 2000);
                                    return false;
                                }
                            }
                        ...
                        $(document).ready(function () {
                            // Configuração do autocomplete para clientes
        ...
        <% } %>
                            ...
                            $('.produto-search').each(function () {
                                const searchInput = $(this);
                                const select = searchInput.next('select.hidden-select');
        ...
        <% } %>
                                        ...
                                        const resultados = produtos.filter(function (produto) {
                                            return produto.codigo.toString().includes(term) ||
                                                    produto.nome.toLowerCase().includes(term);
                                        }).sort(function (a, b) {
                                            return a.nome.localeCompare(b.nome);
                                        });

                                        response($.map(resultados, function (item) {
                                            return {
                                                label: item.codigo + " - " + item.nome + " (R$ " + item.preco.toFixed(2) + ")",
                                    ...
                                    select: function (event, ui) {
                                        ...
                                        for (let i = 0; i < options.length; i++) {
                                            if (options[i].value == ui.item.codigo) {
                                                select[0].selectedIndex = i;
                                                // Disparar o evento change para atualizar preços
                                    ...
                                    return $("<li class='ui-autocomplete-product'>")
                                            .append("<div><span class='product-code'>" + item.codigo + "</span> " +
                                                    "<span class='product-name'>" + item.nome + "</span> " +
                                                    "<span class='product-price'>R$ " + item.preco.toFixed(2) + "</span></div>")
                                            .appendTo(ul);
                                };
                            });
                        ...
                        function atualizarPreco(select) {
                            ...
                            if (selectedOption && selectedOption.getAttribute("data-preco")) {
                                ...
                                // Atualiza o campo de busca com código e nome
                                row.querySelector('.produto-search').value = codigo + " - " + nome;
                        ...
                        function adicionarItem() {
                            ...
                            // Limpar valores
                            const produtoSearch = novaLinha.querySelector('.produto-search');
                            produtoSearch.value = '';

                            const select = novaLinha.querySelector('select[name="produto_id[]"]');
        ...
        <% }%>
                                ...
                                select: function (event, ui) {
                                    ...
                                    // Atualizar o valor exibido no input
                                    produtoSearch.value = ui.item.value;
                            ...
                            // Focar no campo de busca do novo produto
                            produtoSearch.focus();
                        }
                        ...
                        function abrirVencimentos() {
                            ...
                            if (pedidoId) {
                                // Fecha a janela atual antes de abrir a nova
                                if (window.opener) {
                                    window.close();
                                } else {
                        ...
                        function salvarEAbrirVencimentos() {
                            if (validarFormulario()) {
                                // Fecha a janela do pedido antes de abrir os vencimentos
                                if (window.opener) {
                                    window.close();
                                } else {
                                    // Para o caso de não ser um popup, tenta fechar de outra forma
                                    window.open('', '_self').close();
                                }
