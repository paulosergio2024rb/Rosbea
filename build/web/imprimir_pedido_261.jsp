    ...
    try {
        ...
        if (rsDetalhes.next()) {
            ...
            while (rsVencimentos.next()) {
                ...
                // Somar o prazo à data
                if (dataPedidoVcto != null) {
                    Calendar calendar = Calendar.getInstance();
                    calendar.setTime(dataPedidoVcto);
                    calendar.add(Calendar.DAY_OF_MONTH, prazo); // Adiciona o prazo
                    Date novaDataVcto = calendar.getTime();  // Aqui chamando o método getTime()
                    listaVencimentos.add(sdfPrint.format(novaDataVcto)); // Adiciona na lista formatada
                }
            }
            ...
            while (rsPedidoItens.next()) {
                ...
                sb.append(String.format("%-30.30s %4d %9s %9s\n", nome.toUpperCase(), qtd, df.format(unit), df.format(total))); // Alteração para maiúsculas
            ...
            // Rodapé (REMOVIDO: Mês Ref e agradecimentos)
            sb.append((char) 0x1B).append("a").append((char) 0x01);

            // Resumo das vendas do cliente
            sb.append("\n\nResumo de Compras do Cliente:\n");
            sb.append("===========================================\n");

            String sqlResumo = "SELECT mes_atual, rg, total_quantidade FROM vw_relatorio_vendas WHERE rg = ?";
            ...
            try {
                ...
                out.println("<button onclick=\"window.location.href='http://localhost:8080/Rosbea/novos_pedidos.html'\">Voltar para o Pedido</button></body></html>");
            } catch (Exception e) {
